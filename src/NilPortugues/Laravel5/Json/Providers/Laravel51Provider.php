...
class Laravel51Provider
{
    ...
    {
        return function ($app) {
            $parsedRoutes = Cache::rememberForever('nilportugues.json.mapping', function () use ($app) {
                return $this->parseRoutes(new Mapper($app['config']->get('nilportugues.json')));
            });

            return new JsonSerializer(new JsonTransformer($parsedRoutes));
        };
    }
